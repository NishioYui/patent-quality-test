...
if COUNT_INPUT_TOKENS or COUNT_TOKENS_ONLY:
    ...
    token_file_path = os.path.join(
        RUN_DIR,
        f"token_input_spec_{os.path.basename(case_file)}_{run_id}.json"
    )
    ...
    # stdout を汚さない（run_case_allがstdoutをパス扱いしても壊れない）
    print(f"[token_count] spec input_tokens={input_tokens_counted} saved={token_file_path}", file=sys.stderr)

    # 計測だけして終了したい時（※ run_case_all では使わない）
    if COUNT_TOKENS_ONLY:
        print(json.dumps({"step": "spec", "input_tokens": input_tokens_counted, "token_file": token_file_path}, ensure_ascii=False))
        raise SystemExit(0)
# ==========================================================
...
# --- Save usage ---
usage_path = os.path.join(RUN_DIR, f"usage_spec_{os.path.basename(case_file)}_{run_id}.json")
...
cost_path = os.path.join(RUN_DIR, f"cost_spec_{os.path.basename(case_file)}_{run_id}.json")
...
# --- Save ---
out_path = os.path.join(RUN_DIR, f"spec_{os.path.basename(case_file)}_{run_id}.json")
